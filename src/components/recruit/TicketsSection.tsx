...
export default function TicketsSection({ currentDomains }: { currentDomains: string[] }) {
    ...
    return (
        <div
            className="relative min-w-0 overflow-hidden border-2 border-red bg-white p-6 md:p-8"
        >
            ...
            {history.length > 0 && (
                <div className="mt-6 pt-6 border-t border-black/10 space-y-2">
                    ...
                    {history.map((t) => (
                        <div key={t.id} className="border border-black/15 px-4 py-3 bg-black/[0.03]">
                            <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between sm:gap-3">
