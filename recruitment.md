...
- A **Show graph** button (2026-09-01) toggles an inline students-per-mark distribution above the table (recharts, already a dependency). It is built from `visibleRows`, so **every** filter already applies - narrowing to Year 2 + Female re-shapes it with no extra plumbing. Two deliberate choices: recruits with no mark yet are **excluded, not counted as zero** (mid-entry they would pile hundreds onto the 0 bar and drown the real shape; the caption reports `N of M marked` instead), and the x-axis runs 0 → the highest mark **actually scored under that filter**, not 0 → 100, so a domain topping out at 25 gets 51 half-mark bars rather than 201 mostly-empty ones. Buckets are 0.5 wide throughout; since `marks` is numeric restricted to half steps, `Math.round(mark * 2)` is an exact bucket index with no float drift. A row of stat tiles sits above the chart - Marked, Average, Median, Std dev, Lowest, Highest and P25/P75 - computed over the same filtered set. Percentiles use linear interpolation between closest ranks (Excel's PERCENTILE.INC, which is also Postgres' `percentile_cont`), and the spread is the **population** standard deviation, not the sample one: these are every marked recruit under the filter, not a sample drawn from a larger pool. Stats are descriptive rather than stored marks, so they are deliberately NOT snapped to half steps - the median of an even-sized set can legitimately be 19.25 - and render trimmed to 2dp with trailing zeros dropped. Verified against Postgres' own `percentile_cont`/`stddev_pop` on live data
...
**One table, no tabs** - the old "Portfolio Domains" tab was removed along with the portfolio track.

Columns (2026-09-01): **Name, Reg No, Year, Domain, Marks, Status, Called, Override**. Name, Reg No, Year, Domain, Marks and Status are all sortable.

- **Year** and **Marks** were surfaced as real columns on 2026-09-01; both were previously only reachable by expanding a row. Marks is the number the whole page turns on, so sorting by it next to Status makes it immediately obvious who sits just under a cutoff, which is the main reason to override one.
- Year sorts **numerically**, not as the "1"/"2" strings the column stores, so the order stays correct if a third year is ever added (a plain string sort would put "10" before "2").
- A null in Marks means nobody has marked that recruit yet, which is **not** a zero. It renders as a dash so the distinction stays visible, and `compareBy` sorts nulls last in both directions so an unmarked recruit never floats to the top as if they scored 0.

Filters: domain and status (both server-side query params), plus **gender** and **year** (both client-side, added 2026-09-01). Gender and year live on the joined `recruit_accounts` row rather than on `recruit_shortlist_status`, so they cannot ride the server-side query the way domain and status do. Both are nullable/free-text tolerant: a value that is neither option still shows under "All". Gender stays filter-only and is deliberately not rendered as a column.
...
### Walk-in exam attendance, and interview waiting-list detail (2026-09-03)

Migration **021** applied via the Supabase MCP `apply_migration` tool, mirrored into `recruit-schema.sql`. Scanner locked to Walk-in Exam + Interview Check-In today (see the dated lock-comment history above `MODE_OPTIONS` in `src/app/recruit-scanner/page.tsx`).

1. **Walk-in exam (migration 021).** A catch-up sitting for a recruit who already selected a domain but sat neither Day 1 nor Day 2 - typically written during interview day. `recruit_exam_attendance.day` is now nullable; a walk-in row has `day = null, is_walkin = true`, a normal row has `day in (1,2), is_walkin = false`, enforced by a CHECK so the two states can never be ambiguous or mixed. Eligibility is **unchanged** from Day 1/2 - the recruit must still have selected the domain, no bypass - walk-in only changes WHEN it was sat. Writes into the exact same table via the exact same unique key and "already scanned" guard as Day 1/2, so no new duplicate-prevention logic was needed. New scan mode `exam_walkin` in `POST /api/admin/recruitment/scan`, no confirm/force step (unlike interview walk-ins, there's no ambiguous judgment call - either the two existing checks pass and it's marked, or they don't and it's rejected).
2. **Every place that reads `recruit_exam_attendance.day` was audited and fixed** so a walk-in row is never silently dropped or misreported as absent: the scan roster (`/api/admin/recruitment/scan/roster`), the exam check-in board (new **Walk-in** tab alongside Day 1/Day 2/All, `?day=walkin` on the API), analytics (`by_day`/`by_domain_day` gained a `walkin` bucket so `day_1 + day_2 + walkin` always equals the total, same discipline as every other "must add up" fix in this doc), and the Marks page's attendance badge (a walk-in now shows an amber "Walk-in" chip - getting this wrong in the "shows Absent" direction would have hidden that the recruit actually wrote the exam, which is worse than any other failure mode here).
3. **A narrow member permission for walk-in shortlist decisions.** `POST /api/admin/recruitment/walkin-shortlist` lets a `member` (not just lead/admin) decide shortlisted/not-shortlisted, but ONLY for a recruit with a walk-in exam attendance row for that domain - verified server-side, not just hidden in the UI, so it cannot be used as a general override for anyone else. `method = 'walkin_manual'`, skipped by the compute engine the same way `'manual_override'` already is, so a later bulk "Run Shortlist" never clobbers it.
4. **The scanner's Walk-in Exam mode has an inline follow-up panel**, right after a successful scan: enter marks (shared `parseMarksValue`/half-step validation) → Save → only then do "Shortlist"/"Not Shortlisted" buttons unlock, calling the new endpoint above. One recruit at a time; a new scan replaces the panel. This exists specifically so a recruit who writes a makeup exam and gets marked can be given a same-day decision without a separate trip to the Marks/Shortlist pages.
5. **Interview waiting-list rows are now expandable** to the full recruit profile (name, reg no, year, department, gender, hosteller/day-scholar + block/area, LinkedIn, domains applied with shortlist-star highlighting, exam marks) - reusing the existing `RecruitProfileCard` already used for "Now Serving", with zero new data fetching (`token.recruit` was already the full profile in memory, just not rendered for waiting rows before). Drag-to-reorder and the "Call to X" buttons are unaffected - the expand toggle is a separate click target from both.
6. **Two important clarifications surfaced while scoping this**, worth remembering: "walk-in exam" and "walk-in interview" are two independent concepts that happen to share a word - a recruit who legitimately clears cutoff via a walk-in exam gets a completely ordinary interview check-in (no `is_walkin` token, no confirm prompt), because the interview walk-in flag specifically means "not shortlisted, let in as an exception" and does not fire for someone who is shortlisted. And marks entry was already member-accessible before this change; only the shortlist decision needed the new narrow permission.
...
- `supabase/recruit-migration-0NN-*.sql` - the numbered migrations, apply in order (highest is 021). 004 = table_number/queue_position/denormalized sub_domain; 005 = the `deferred` enum value; 013 = gender-scoped cutoffs; 014 (2026-08-16) = `recruit_public_chat_requests`, the rate-limit table for the public chatbot; 016 = drop year '3'; 018 (2026-09-01) = year-scoped cutoffs; 019 (2026-09-01) = `recruit_marks.note`; 020 (2026-09-01) = half marks (marks + cutoffs integer -> numeric); 021 (2026-09-03) = walk-in exam attendance (`recruit_exam_attendance.is_walkin`, `day` made nullable). Note 004 and 005 each got used twice by different files - a pre-existing collision, don't reuse a number again
